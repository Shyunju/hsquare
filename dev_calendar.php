...
<body>
    ...
    <div id='right_side'>
        ...
        <input type="button" name="confirm" value="회원정보" onClick="location.href='dev_mem_list.php'"><br>
    </div>
    <div class="container">
        <table class="table table-bordered table-responsive">
            <tr align="center" >
                <td>
                    <a href=<?php echo 'dev_calendar.php?year='.$preyear.'&month='.$month . '&day=1'; ?>>◀◀</a>
                </td>
                <td>
                    <a href=<?php echo 'dev_calendar.php?year='.$prev_year.'&month='.$prev_month . '&day=1'; ?>>◀</a>
                </td>
                ...
                <td>
                    <a href=<?php echo 'dev_calendar.php?year='.$next_year.'&month='.$next_month.'&day=1'; ?>>▶</a>
                </td>
                <td>
                    <a href=<?php echo 'dev_calendar.php?year='.$nextyear.'&month='.$month.'&day=1'; ?>>▶▶</a>
                </td>
            </tr>
            <tr class="info">
                <th hight="30">일</td>
            ...
            for ($j = 0; $j < 7; $j++) {
                ...
                if (!(($i == 1 && $j < $start_week) || ($i == $total_week && $j > $last_week))) {
                    ...
                    //해당 년, 달 의 캘린더 디비 행 가져오는 쿼리문
                    $sql="SELECT * FROM lec_info WHERE yyy='$year' AND mmm='$month'AND ddd='$day'";
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_array($result);

                    if(isset($row)){
                        $sql="SELECT * FROM umaster WHERE id={$row['umaster_id']}";
                        $result = mysqli_query($conn, $sql);
                        $row = mysqli_fetch_array($result);
                    ?>
                        <br>
                        <span id='reserv'> 예약됨 <br> <?=$row['uname']?>님</span>
                    <?php
                    }
                    // 14. 날짜 증가
                    $day++;
